...
// Fragment shader - fractal computation and coloring
...
void main() {
    ...
    float real = u_center.x + (uv.x - u_resolution.x / 2.0) / u_zoom;
    float imag = u_center.y + (uv.y - u_resolution.y / 2.0) / u_zoom;
    ...
    if (iterations >= u_maxIterations) {
        ...
        // Smooth coloring
        float smoothIter = float(iterations) - log2(log2(zr2 + zi2)) + 4.0;
...
export class WebGLRenderer {
    ...
    render(params: RenderParams): void {
        ...
        gl.uniform2f(this.uniforms.u_center, params.centerReal, params.centerImag);
